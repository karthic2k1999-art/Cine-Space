...
(() => {
  ...
  async function initEngine() {
    try {
      ...
      // The @ffmpeg/ffmpeg UMD bundle spawns its own internal Worker (file "814.ffmpeg.js").
      // Loading that Worker directly from a different origin (unpkg.com) is blocked by the
      // browser's Same-Origin policy for Workers ("SecurityError: Failed to construct 'Worker'").
      // Fix: pull it down and hand FFmpeg a same-origin blob: URL instead, exactly like we
      // already do for the core engine + wasm binary below.
      await ffmpeg.load({
      ...
      showError(
        "The real processing engine could not be loaded. Check your internet connection and reload the page.",
        err
      );
    }
  }
  ...
  function applyLook(lookId, resetSliders) {
    const look = LOOKS.find((l) => l.id === lookId) || LOOKS[0];
    state.look = look.id;
    if (resetSliders) {
      state.sharpness = look.sharpness;
      state.adv = { ...look.adv };
    }
  ...
  function buildFilterChain() {
    ...
    // Microcontrast + highlight/shadow protection via a bounded eq() pass.
    const micro = 1 + (adv.microcontrast / 100) * 0.18;
    const gamma = 1 - (adv.highlight / 100) * 0.06 + (adv.shadow / 100) * 0.04;
    if (adv.microcontrast > 0 || adv.highlight > 0 || adv.shadow > 0) {
      filters.push(`eq=contrast=${micro.toFixed(3)}:gamma=${clamp(gamma, 0.85, 1.15).toFixed(3)}`);
    }
